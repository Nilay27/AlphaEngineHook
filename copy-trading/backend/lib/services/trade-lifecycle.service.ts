...
import { tradeConfirmationsTable, subscriptionsTable, addressMappingsTable } from '@/db/schema';
...
export class TradeLifecycleService extends EventEmitter {
  ...
  async broadcastTrade(params: BroadcastTradeParams): Promise<string[]> {
    ...
    const encryptedSubscribers = await this.getEncryptedSubscribers(generatorAddress);
    ...
    for (const encryptedAddress of encryptedSubscribers) {
      const [confirmation] = await db
        ...
        .values({
          ...
          tradeId: ethers.id(`${generatorAddress}-${encryptedAddress}-${Date.now()}`),
          protocolId,
          action,
          executionData,
          expiryTime,
  ...
  async getTradesByConsumer(
    ...
    const mappings = await db
      ...
      .where(eq(addressMappingsTable.realAddress, consumerAddress));
