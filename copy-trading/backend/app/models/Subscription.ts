...
export class SubscriptionModel {
  ...
  static async findByStrategy(strategyId: number, isActive?: boolean): Promise<Subscription[]> {
  ...
  static async findBySubscriber(subscriberWallet: string, isActive?: boolean): Promise<Subscription[]> {
    const conditions = [eq(subscriptions.subscriberWallet, subscriberWallet)]
    if (isActive !== undefined) {
      conditions.push(eq(subscriptions.isActive, isActive))
    }
  ...
  static async findOne(strategyId: number, subscriberWallet: string): Promise<Subscription | null> {
    const [subscription] = await db
      ...
      .where(
        and(
          ...
          eq(subscriptions.subscriberWallet, subscriberWallet)
        )
      )
      .limit(1)
  ...
  static async findWithStrategy(subscriberWallet: string): Promise<any[]> {
    return await db
      ...
      .where(eq(subscriptions.subscriberWallet, subscriberWallet))
      .orderBy(desc(subscriptions.createdAt))
  }

  // Update subscription
  static async update(id: number, data: Partial<NewSubscription>): Promise<Subscription | null> {
    const [subscription] = await db
  ...
  static async unsubscribe(strategyId: number, subscriberWallet: string): Promise<boolean> {
