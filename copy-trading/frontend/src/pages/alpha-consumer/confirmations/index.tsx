...
const ConsumerConfirmationsPage: React.FC = () => {
  ...
  const handleApprove = async (confirmationId: string) => {
    ...
    try {
      ...
      // For now, we'll stub the contract interaction
      // In production, this would execute the actual trade on-chain
      const stubTxHash = `0x${Date.now().toString(16)}`;
      
      // Use confirmations service to mark the confirmation as executed
      await confirmationsService.completeConfirmation(
        confirmationId,
        true, // isExecuted
        stubTxHash // executionTxHash
      );
